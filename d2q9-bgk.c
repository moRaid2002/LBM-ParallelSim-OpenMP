/*
 *git : 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
...
*/
int timestep(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* restrict obstacles, float* restrict av_vels, const int iteration );
int accelerate_flow(const t_param params, t_speed* restrict cells, int* restrict obstacles);
int propagate(const t_param params, t_speed* restrict cells, t_speed* restrict tmp_cells, int* restrict obstacles);
...
{
  accelerate_flow(params, cells, obstacles);
  av_vels[iteration] = collision(params, cells, tmp_cells, obstacles);
  return EXIT_SUCCESS;
}
...
#pragma omp simd
    ...
    float t4 ;
     ...
     float t7;
   float t8;
      if (obstacles[jj*params.nx + ii] != 0) {
        t0 = s0;
       t1 = s3;
       t2 = s4;
        t3 = s1;
       ...
       t6 = s8;
        ...
        t0 = s0 + params.omega * (d_equ[0] - s0);
         t1 = s1 + params.omega * (d_equ[1] - s1);
         t2 = s2 + params.omega * (d_equ[2] - s2);
        t3 = s3 + params.omega * (d_equ[3] - s3);
         t4 = s4 + params.omega * (d_equ[4] - s4);
        t5 = s5 + params.omega * (d_equ[5] - s5);
        t6 = s6 + params.omega * (d_equ[6] - s6);
        t7 = s7 + params.omega * (d_equ[7] - s7);
         t8 = s8 + params.omega * (d_equ[8] - s8);
      }
